...
def main():
    ...
    sv = SourceFileLoader("mv_sv", os.path.join(os.path.dirname(__file__), "test_raw_mv_svector.py")).load_module()
    ...
    print("  mv arithmetic intensity = O(1) flops/byte → memory-bound regime")
    if np.max(utils) < 50:
        print(f"  ⚠ {np.max(utils):.0f}% peak — kernel launch overhead dominates at small shapes")
        print("    (larger M/K pushes utilization higher; 256x256 is tiny)")
    else:
