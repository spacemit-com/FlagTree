...
@tle.raw_kernel
def cross_entropy_1d_kernel(X: tle.mem(f32), out: tle.mem(f32, out=True), N: tle.index, target: tle.index):
    nvl = tle.vconfig(-1, 1)
    Nfloor = (N // nvl) * nvl
...
def _ref_cross_entropy(logits: torch.Tensor, target: int) -> float:
    return torch.nn.functional.cross_entropy(logits.unsqueeze(0), torch.tensor([target])).item()
