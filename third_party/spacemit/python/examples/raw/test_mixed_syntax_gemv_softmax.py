...
@tle.raw_kernel
def gemv_stage(Mat: tle.mem(f16), vec: tle.mem(f16), scores: tle.mem(f32, out=True), K: tle.index, N: tle.index):
    nvl = tle.vconfig(-1, 1)
    Kfloor = (K // nvl) * nvl
...
def _run(N, K):
    ...
    gemv_softmax_host[(1, )](Mat.contiguous().reshape(-1), vec.contiguous(), scores, out, K, N)
